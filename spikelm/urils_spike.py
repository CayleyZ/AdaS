...
class ElasticSpike(torch.autograd.Function):
    """
    ...
    def forward(ctx, input, alpha, num_bits, layerwise):
        ...
        if num_bits == 1:
            q_w = input.sign()  ################################## 1 bit
        else:
            q_w = (input / alpha).round().clamp(Qn, Qp)  ###################### 2bit
        w_q = q_w * alpha
        return w_q
...
#         self.act_quantizer = ElasticSpike
...
#         # spike = torch.zeros_like(input[0]).cuda()
...
class SpikeLinear(nn.Linear):
    ...
    def forward(self, input):
        ...
        self.firing_rate = ((output != 0).float().sum().item()/output.numel() + self.cnt * self.firing_rate)/(self.cnt + 1)
        self.cnt += 1
        
        out = nn.functional.linear(output, weight)
        if not self.bias is None:
            # print(self.mask.shape, self.weight.shape, self.bias.shape)
            out += (self.bias.view(1, -1) * self.mask[:, 0].view(1, -1)).expand_as(out)
            # print((self.bias.view(1, -1) * self.mask[0].view(1, -1)).expand_as(out))
